...
class AgentFallbackModelConfig(BaseModel):
    """Optional tenant-owned fallback for a single model invocation."""
    ...
    first_chunk_timeout: float = Field(default=8, ge=1, le=120)
...
class AgentSoulModelConfig(BaseModel):
    ...
    model_provider: str = Field(min_length=1, max_length=255)
    model: str = Field(min_length=1, max_length=255)
    fallback: AgentFallbackModelConfig | None = None
