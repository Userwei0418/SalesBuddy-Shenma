...
class AgentLLMInnerService:
    ...
    def _invoke_once(self, prepared: PreparedAgentLLMInvocation) -> Generator[LLMResultChunk, None, None]:
    ...
    def _fallback_for(self, prepared):
        """Load the exact version, never the latest mutable published pointer."""
        from models.agent import Agent, AgentConfigDraft, AgentConfigSnapshot
        caller = prepared.request.caller
        if caller.agent_mode != "agent_app" or not caller.agent_id or not caller.agent_config_version_id:
            return None
        with self._session_factory() as session:
            agent = session.get(Agent, caller.agent_id)
            if agent is None or agent.tenant_id != caller.tenant_id or agent.app_id != caller.app_id:
                return None
            cls = AgentConfigSnapshot if caller.agent_config_version_kind == "snapshot" else AgentConfigDraft
            version = session.get(cls, caller.agent_config_version_id)
            if version is None or version.tenant_id != caller.tenant_id or version.agent_id != agent.id:
                return None
            model = version.config_snapshot_dict.get("model") or {}
            target = prepared.request.target
            if (model.get("model_provider"), model.get("model")) != (target.provider, target.model):
                return None
            fallback = model.get("fallback")
            if not fallback or (fallback["model_provider"], fallback["model"]) == (target.provider, target.model):
                return None
            return fallback

    def invoke(self, prepared):
        from services.agent_model_fallback import invoke_with_fallback
        fallback = self._fallback_for(prepared)
        if fallback is None:
            yield from self._invoke_once(prepared)
            return

        def secondary():
            request = prepared.request.model_copy(deep=True)
            request.target.provider = fallback["model_provider"]
            request.target.model = fallback["model"]
            request.target.model_parameters = {
                key: value for key, value in fallback.get("model_settings", {}).items()
                if value is not None and key != "stop"
            }
            request.target.stop = fallback.get("model_settings", {}).get("stop")
            # Resolve credentials and model availability inside the same tenant.
            return self._invoke_once(self.prepare(request))

        yield from invoke_with_fallback(
            lambda: self._invoke_once(prepared), secondary,
            timeout=fallback.get("first_chunk_timeout", 8),
            context={"agent_id": prepared.request.caller.agent_id,
                     "run_id": prepared.request.caller.agent_run_id,
                     "primary": prepared.request.target.model,
