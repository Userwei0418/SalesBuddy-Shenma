...
from sales_contracts.contracts.opportunity_candidate import validate_candidate

from sales_contracts.domain.visit_contract import ensure_visit_result

from sales_contracts.integrations.senseaudio import SenseAudioError

from sales_contracts.services.agent_platform.routing import InvalidAgentResult
...
def validate_run_result(run, result, facts):
    from sales_contracts.services.agent_run.contract import ensure_instant_summary_contract
    from sales_contracts.services.agent_run.materialize import RISK_TYPE_CODES, SEVERITY_LABELS, TASK_PRIORITY_CODES
    ...
    try:
        if run.mode == "visit_entry":
            if facts.get("visit_stage"):
                from sales_contracts.contracts.visit_flow import validate_stage_result

                return validate_stage_result(result, facts)
            from sales_contracts.domain.visit_review import review_text
        ...
        if run.mode in {"chatbi", "customer_chatbi"}:
            ...
            from sales_contracts.domain.follow_up_schedule import validate_today_task_dates
