...
class _AgentProcessRecorder:
    ...
    def trim_answer_suffix(self, final_answer: str) -> None:
        ...
        db.session.commit()
        if self._queue_manager.is_stopped():
            return
...
class AgentAppRunner:
    ...
    def run(
        ...
        *,
        ...
        from core.app.apps.agent_app.lifecycle import record, stamp
        from core.app.apps.agent_app.output_validation import policy_for, OutputValidationError
        from core.app.apps.agent_app.business_validation import prepare_business
        output_policy = policy_for(dify_context.app_id, agent_config_snapshot_id)
        try:
            business_context = prepare_business(output_policy, query, agent_id, agent_config_snapshot_id) if output_policy else None
        except OutputValidationError as exc:
            record(message_id, dify_context.app_id, status='failed',
                   output_validation={'status': 'failed', 'code': str(exc), **getattr(exc, 'details', {})})
            raise InvokeBadRequestError('Agent output validation failed: ' + str(exc)) from None
        record(message_id, dify_context.app_id, backend_submit_at=stamp())
        create_response = self._agent_backend_client.create_run(runtime.request)
        record(message_id, dify_context.app_id, backend_run_id=create_response.run_id, backend_accepted_at=stamp())
        terminal, process_recorder = self._consume_stream(
            ...
            buffer_json_output=output_policy is not None,
        )
        ...
        from core.app.apps.agent_app.lifecycle import record, stamp
        record(message_id, dify_context.app_id, backend_completed_at=stamp())
        answer = self._terminal_output_to_answer(terminal.output)
        if output_policy is not None:
            from core.app.apps.agent_app.output_validation import normalize_and_validate, OutputValidationError
            try:
                answer, validation = normalize_and_validate(answer, output_policy['schema'])
                if business_context is not None:
                    from core.app.apps.agent_app.business_validation import validate_business
                    validation.update(validate_business(answer, business_context, agent_config_snapshot_id))
            except OutputValidationError as exc:
                record(message_id, dify_context.app_id, status='failed',
                       output_validation={'status': 'failed', 'code': str(exc), **getattr(exc, 'details', {})})
                raise InvokeBadRequestError('Agent output validation failed: ' + str(exc)) from None
            record(message_id, dify_context.app_id, output_validation=validation)
        try:
    ...
    def _consume_stream(
        ...
        *,
        ...
        def cancel_once(**kwargs):
            ...
            from core.app.apps.agent_app.lifecycle import record, stamp
            record(message_id, dify_context.app_id, status="cancelled", cancel_requested_at=stamp())
            self._cancel_run(run_id, **kwargs)


        def persist_answer_text(content_delta: str) -> None:
            if buffer_json_output:
                return
            try:
        ...
        try:
            ...
            for public_event in public_events:
                if first_event:
                    from core.app.apps.agent_app.lifecycle import record, stamp
                    record(message_id, dify_context.app_id, backend_first_event_at=stamp())
                    first_event = False
                ...
                for internal_event in self._event_adapter.adapt(public_event):
                    ...
                    if internal_event.type in (
                        ...
                        if isinstance(internal_event, AgentBackendAgentMessageDeltaInternalEvent):
                            if first_text:
                                from core.app.apps.agent_app.lifecycle import record, stamp
                                record(message_id, dify_context.app_id, first_text_at=stamp())
                                first_text = False
            ...
            cancel_once(after=last_event_id, session_scope=session_scope, binding_id=binding_id, message_id=message_id)
            raise
