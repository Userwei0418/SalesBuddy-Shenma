...
class AgentAppGenerator(MessageBasedAppGenerator):
    def generate(
        ...
        *,
        ...
        record(message.id, app_config.app_id, status="running", accepted_at=stamp(),
               task_id=application_generate_entity.task_id,
        ...
        if streaming:
            from core.app.apps.agent_app.durable_response import durable_response
            from libs.flask_utils import preserve_flask_contexts

            response_app = current_app._get_current_object()
            response_context = contextvars.copy_context()
            response_message_id = message.id
            response_app_id = app_config.app_id

            def persistence_error(exc):
                from core.app.apps.agent_app.lifecycle import record, stamp
                logger.exception("Agent response persistence failed message=%s", response_message_id)
                record(response_message_id, response_app_id, status="failed",
                       finished_at=stamp(), error_type=type(exc).__name__)

            response = durable_response(
                response,
                context=lambda: preserve_flask_contexts(response_app, context_vars=response_context),
                on_error=persistence_error,
            )
        converted = AgentAppGenerateResponseConverter.convert(response=response, invoke_from=invoke_from)
        if invoke_from == InvokeFrom.SERVICE_API:
            from core.app.apps.agent_app.lifecycle import started_stream
            return started_stream(converted, task_id=application_generate_entity.task_id,
        ...
        *,
        ...
        with preserve_flask_contexts(flask_app, context_vars=context):
            try:
                ...
                from core.app.apps.agent_app.lifecycle import record, stamp
                record(message_id, app_config.app_id, status="cancelled", finished_at=stamp())
            except Exception as e:
                from core.app.apps.agent_app.lifecycle import record, stamp
                record(message_id, app_config.app_id, status=("timed_out" if isinstance(e, TimeoutError) else "failed"), finished_at=stamp(), error_type=type(e).__name__)
