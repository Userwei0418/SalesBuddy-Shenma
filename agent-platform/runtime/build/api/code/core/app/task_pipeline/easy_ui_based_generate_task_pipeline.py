...
class EasyUIBasedGenerateTaskPipeline(BasedGenerateTaskPipeline[EasyUIAppGenerateEntity]):
    ...
    def _save_message(
        ...
        *,
        ...
        message_stmt = select(Message).where(Message.id == self._message_id).with_for_update()
        ...
        if "agent_execution" in existing_metadata:
            from core.app.apps.agent_app.lifecycle import merge_execution, stamp
            metadata["agent_execution"] = existing_metadata["agent_execution"]
            state = "cancelled" if preserve_existing_usage else "succeeded"
            metadata = merge_execution(metadata, {"status":state, "finished_at":stamp()})
            if metadata["agent_execution"]["status"] in ("cancelled", "failed", "timed_out"):
                from models.enums import MessageStatus
                message.status = MessageStatus.ERROR
                message.error = "Agent execution " + metadata["agent_execution"]["status"]
