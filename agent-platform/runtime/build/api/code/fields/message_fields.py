...
class MessageListItem(ResponseModel):
    ...
    execution: dict[str, JSONValueType] | None = Field(default=None, validation_alias="message_metadata_dict")

    @field_validator("execution", mode="before")
    @classmethod
    def _execution_metadata(cls, value):
        return value.get("agent_execution") if isinstance(value, dict) else None

    @computed_field
    def execution_status(self) -> str:
        if self.execution and self.execution.get("status"):
            return str(self.execution["status"])
        if self.status == "error":
            return "failed"
        # Legacy normal + blank is not evidence of successful completion.
        return "unknown" if not self.answer else "legacy_unverified"
    ...
    @field_validator("retriever_resources", mode="before")
    @classmethod
    def _normalize_retriever_resources(cls, value: object) -> object:
        # Non-retrieval Agent messages may have metadata without this key.
        # Keep the public array contract; preserve validation of malformed values.
