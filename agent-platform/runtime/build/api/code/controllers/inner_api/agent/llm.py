...
class AgentLLMInvokeApi(Resource):
    ...
    def post(self) -> Response:
        ...
        def generate() -> Generator[str, None, None]:
            ...
            tag = (caller.agent_run_id, caller.invocation_id, payload.target.model)
            log.info("agent_model_start run=%s invocation=%s model=%s", *tag)
            first = True
            try:
                for chunk in service.invoke(prepared):
                    if first:
                        log.info("agent_model_first_chunk run=%s invocation=%s model=%s elapsed=%.3f", *tag, time.monotonic()-started)
                        first = False
                    envelope = {"code": 0, "message": "", "data": chunk.model_dump(mode="json")}
                    yield f"data: {json.dumps(envelope, ensure_ascii=False, separators=(',', ':'))}\n\n"
                log.info("agent_model_complete run=%s invocation=%s model=%s elapsed=%.3f", *tag, time.monotonic()-started)
                ...
                log.warning("agent_model_error run=%s invocation=%s model=%s error_type=%s elapsed=%.3f", *tag, type(exc).__name__, time.monotonic()-started)
                error = {
