...
# Deployment extension: authenticated, account-scoped workspace provisioning.
from pydantic import ConfigDict, ValidationError
from werkzeug.exceptions import BadRequest
from services.super_fde_workspace_creation import can_create_workspace, create_workspace_for_account


class SuperFdeWorkspaceCreatePayload(BaseModel):
    ...
    def valid_name(cls, value: str) -> str:
        if any(ord(char) < 32 for char in value):
            raise ValueError("Workspace name cannot contain control characters")
        return value


class SuperFdeWorkspaceCapabilityResponse(ResponseModel):
    allowed: bool


class SuperFdeWorkspaceCreateResponse(ResponseModel):
    id: str
    name: str
    created: bool


register_schema_models(console_ns, SuperFdeWorkspaceCreatePayload)
register_response_schema_models(console_ns, SuperFdeWorkspaceCapabilityResponse, SuperFdeWorkspaceCreateResponse)


@console_ns.route("/super-fde/workspace-creation")
class SuperFdeWorkspaceCreationApi(Resource):
    @console_ns.response(200, "Creator capability", console_ns.models[SuperFdeWorkspaceCapabilityResponse.__name__])
    @setup_required
    @login_required
    @account_initialization_required
    @with_current_user
    def get(self, current_user: Account):
        return {"allowed": can_create_workspace(current_user.id)}

    @console_ns.expect(console_ns.models[SuperFdeWorkspaceCreatePayload.__name__])
    @console_ns.response(200, "Workspace", console_ns.models[SuperFdeWorkspaceCreateResponse.__name__])
    @setup_required
    @login_required
    @account_initialization_required
    @with_current_user
    ...
    def post(self, session: Session, current_user: Account):
        try:
            args = SuperFdeWorkspaceCreatePayload.model_validate(request.get_json(silent=True) or {})
        except ValidationError as exc:
            raise BadRequest("Workspace name must be 1–80 characters; unsupported fields are not allowed.") from exc
        tenant, created = create_workspace_for_account(current_user, args.name, session=session)
        return {"id": str(tenant.id), "name": tenant.name, "created": created}
