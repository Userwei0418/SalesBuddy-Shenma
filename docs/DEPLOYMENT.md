...
## 当前版本（2026-09-24 20:00 后）

销售 API/Worker 已同步至 1.0.9 及后台性能补丁，运行 `bb02f63` / V151；13 个测试账号已验证，中台更新 6 项提示词，4 项核心 Worker 联调通过，小程序已完成客户 AppID 编译预览。完整状态、证据及边界见 [本次同步记录](SYNC_1.0.9_20260924.md)。下文为历史过程，以本节与本次记录为准。

## 最新补充（2026-09-24 14:25 北京时间）

- 客户 AppID `wx2824bdeb58528fd8` 已配置，微信开发者工具已完成客户 AppID 编译预览并生成二维码；尚未完成真机登录、录音和上传验收。
- 两个域名已应用客户提供的国科云正式证书，公网常规 CA 校验通过。证书均于 2026-10-24 23:59:59（北京时间）到期；续签与自动下发仍需确认。详情见 TLS.md。
- 用户已授权创建专用测试销售账号，密码登录、首次改密、销售助手主页访问和后台管理权限拒绝检查通过。凭据仅通过仓库外私有文件交付。
- 原有四项中台试点加入该测试账号，保持原 48 小时试点到期时间；未扩大到全体用户。无关联客户的商机草稿请求被业务校验拒绝，完整业务闭环尚未通过。
- 下文保留此前部署过程；其中 AppID 待提供、临时自签证书和测试账号待确认属于历史状态，以本节为准。
