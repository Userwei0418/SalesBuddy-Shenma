...
- 销售 28899 已通过临时公共证书验证并返回正确 API 版本；正式可信证书尚未签发。中台已安装并通过指定实例证书的 HTTPS 验证。
...
DNS 客户端配置完成后，先通过测试 CA 验证自动创建/清理记录，再正式签发对应域名证书。Certbot 保存验证参数后，系统定时器将复用该参数自动续期。中台已经安装，签发后可通过既有部署钩子将证书应用到运行实例。
