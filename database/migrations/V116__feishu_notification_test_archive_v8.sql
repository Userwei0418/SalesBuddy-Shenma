...
DO $$ BEGIN
 IF NOT EXISTS(SELECT 1 FROM pg_roles WHERE rolname=current_user AND (rolsuper OR rolbypassrls)) THEN
  RAISE EXCEPTION 'V116 requires the audited migration owner with complete RLS visibility';
 END IF;
END $$;
...
CREATE OR REPLACE FUNCTION security.feishu_notification_test_archive_v2(p_descriptor jsonb,p_expected_plan text DEFAULT NULL) RETURNS jsonb
...
BEGIN
  ...
  IF row_security_active('insight.business_advice'::regclass)
   ...
   OR EXISTS(SELECT 1 FROM ops.schema_migration WHERE version ~ '^V[0-9]+$'
     AND substring(version FROM 2)::int>116) THEN
  ...
  SELECT v.id,v.version_no INTO vid,visit_version
   ...
   WHERE v.workspace_id=ws AND v.customer_id=cid
  ...
  FOREACH dependent_table IN ARRAY ARRAY[
    ...
    ELSIF dependent_table='crm.business_change' AND EXISTS(SELECT 1 FROM crm.business_change
      ...
      IF (SELECT count(*) FROM crm.business_change
          WHERE workspace_id=ws AND (customer_id=cid OR opportunity_id=oid))<>1
         OR NOT EXISTS(SELECT 1 FROM crm.business_change
          WHERE workspace_id=ws AND (customer_id=cid OR opportunity_id=oid)
            AND changes::text ILIKE '%'||v_opportunity_name||'%') THEN
        RETURN jsonb_build_object('code','DEPENDENCY_REQUIRES_REVIEW:'||dependent_table);
      END IF;
    ...
    ELSIF dependent_table='insight.business_advice' AND EXISTS(SELECT 1 FROM insight.business_advice
      ...
      IF (SELECT count(*) FROM insight.business_advice
          WHERE workspace_id=ws AND (customer_id=cid OR opportunity_id=oid OR visit_id=vid))<>2
         OR EXISTS(SELECT 1 FROM insight.business_advice
          WHERE workspace_id=ws AND (customer_id=cid OR opportunity_id=oid OR visit_id=vid)
            AND (status<>'succeeded' OR subject_kind<>'customer' OR opportunity_id IS NOT NULL
                 OR visit_id IS NOT NULL OR created_at<day_start OR created_at>=day_end)) THEN
        RETURN jsonb_build_object('code','DEPENDENCY_REQUIRES_REVIEW:'||dependent_table);
      END IF;
    ELSIF dependent_table='insight.business_suggestion' AND EXISTS(SELECT 1 FROM insight.business_suggestion s
      ...
      IF (SELECT count(*) FROM insight.business_suggestion s
          JOIN insight.business_advice a ON a.id=s.advice_id
          WHERE a.workspace_id=ws AND (a.customer_id=cid OR a.opportunity_id=oid OR a.visit_id=vid))<>1
         OR EXISTS(SELECT 1 FROM insight.business_suggestion s
          JOIN insight.business_advice a ON a.id=s.advice_id
          WHERE a.workspace_id=ws AND (a.customer_id=cid OR a.opportunity_id=oid OR a.visit_id=vid)
            AND (a.status<>'succeeded' OR a.subject_kind<>'customer')) THEN
        RETURN jsonb_build_object('code','DEPENDENCY_REQUIRES_REVIEW:'||dependent_table);
      END IF;
    ELSIF dependent_table='insight.sales_competency_review' AND EXISTS(SELECT 1 FROM insight.sales_competency_review r
      ...
      -- The competency review is a shared, user-level daily artifact rather than
      -- a child of this four-record smoke-test batch. Preserve completed shared
      -- reviews; block only an in-flight/failed review that still needs review.
      IF EXISTS(SELECT 1 FROM insight.sales_competency_review r
        JOIN activity.visit v ON v.workspace_id=r.workspace_id AND v.recorder_user_ref_id=r.subject_user_ref_id
        WHERE r.workspace_id=ws AND v.id=vid AND r.status IN ('queued','running','failed')) THEN
        RETURN jsonb_build_object('code','DEPENDENCY_REQUIRES_REVIEW:'||dependent_table);
      END IF;
    ELSIF dependent_table='agent.conversation' AND EXISTS(SELECT 1 FROM agent.conversation
      ...
      -- Ended Agent history is retained with the archived batch; an active
      -- conversation still blocks cleanup until its lifecycle is closed.
      IF EXISTS(SELECT 1 FROM agent.conversation
        WHERE workspace_id=ws AND status='active' AND (context @> jsonb_build_object('customer_id',cid::text)
          OR context @> jsonb_build_object('opportunity_id',oid::text)
          OR context @> jsonb_build_object('visit_id',vid::text))) THEN
        RETURN jsonb_build_object('code','DEPENDENCY_REQUIRES_REVIEW:'||dependent_table);
      END IF;
...
INSERT INTO ops.schema_migration(version,description)
 VALUES('V116','固定标识飞书通知联调演示数据的一次性受控归档入口（保留共享复盘与已结束Agent会话）');
COMMIT;
