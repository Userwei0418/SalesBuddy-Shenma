...
-- New objects inherit the deployment's existing application-role grants, never PUBLIC writes.
DO $$ DECLARE r record; BEGIN
 ...
 LOOP EXECUTE format('GRANT SELECT,INSERT,UPDATE ON crm.partner TO %I',r.grantee); END LOOP;
END $$;

CREATE FUNCTION security.ranking_team_group(p_name text) RETURNS text
LANGUAGE sql IMMUTABLE SET search_path=pg_catalog AS $$
 SELECT CASE replace(regexp_replace(trim(p_name),'\s','','g'),'＋','+')
 WHEN '北区' THEN 'north_east' WHEN '东区' THEN 'north_east'
 WHEN '华北' THEN 'north_east' WHEN '华北区' THEN 'north_east'
 WHEN '华东' THEN 'north_east' WHEN '华东区' THEN 'north_east'
 WHEN '北区东区' THEN 'north_east' WHEN '北区+东区' THEN 'north_east' WHEN '北区、东区' THEN 'north_east'
 WHEN '南区' THEN 'south_hkmo' WHEN '华南' THEN 'south_hkmo' WHEN '华南区' THEN 'south_hkmo'
 WHEN '港澳' THEN 'south_hkmo' WHEN '港澳区' THEN 'south_hkmo'
 WHEN '香港' THEN 'south_hkmo' WHEN '澳门' THEN 'south_hkmo'
 WHEN '南区港澳' THEN 'south_hkmo' WHEN '南区+港澳' THEN 'south_hkmo' WHEN '南区、港澳' THEN 'south_hkmo'
 ELSE 'unassigned' END;
$$;

-- Aggregate-only boundary. Rank the complete authorized cohort before hiding other salespeople.
-- This function never returns customer/opportunity IDs, titles or per-record amounts.
CREATE FUNCTION security.business_ranking(p_metric text,p_start date,p_end date,
 ...
 WITH members AS (
  ...
  WHERE u.workspace_id=common.current_workspace_id() AND u.status='active' AND u.deleted_at IS NULL
    AND (common.current_role_code()='manager' OR u.id=common.current_user_ref_id()
      OR (common.current_role_code()='supervisor' AND security.supervises_team(t.id))
      OR (common.current_role_code()='sales' AND EXISTS(SELECT 1 FROM platform.team_membership own
        WHERE own.workspace_id=u.workspace_id AND own.user_ref_id=common.current_user_ref_id()
          AND own.team_id=t.id AND clock_timestamp()>=own.valid_from AND clock_timestamp()<own.valid_to)))
 ), events AS (
  ...
  UNION ALL
  SELECT m.id,m.team_id,1::numeric,c.id
  ...
  UNION ALL SELECT *,NULL::bigint AS rank FROM group_values WHERE code='unassigned'
   ...
   'scope',CASE WHEN common.current_role_code()='sales' OR p_self_only THEN 'self'
     WHEN common.current_role_code()='supervisor' THEN 'team' ELSE 'workspace' END,
   'cohort',CASE WHEN common.current_role_code()='sales' THEN 'team_sales'
   ...
   'calculation','confirmed_followup_current_claim_nonlost_opportunity_v1') INTO answer;
 RETURN answer;
END;
$$;
COMMENT ON FUNCTION security.business_ranking(text,date,date,integer[],boolean) IS
 '服务端完整排名；一线仅返回本人统计和同团队销售名次，管理者返回授权聚合；不输出他人商机或客户明细';
INSERT INTO ops.schema_migration(version,description) VALUES('V064','原生权限聚合排名与伙伴目录运行角色授权');
COMMIT;
