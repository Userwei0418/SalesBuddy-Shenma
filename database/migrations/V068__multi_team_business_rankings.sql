 ...
 WITH members AS (
  ...
  WHERE u.workspace_id=common.current_workspace_id() AND u.status='active' AND u.deleted_at IS NULL
    AND (common.current_role_code()='manager' OR u.id=common.current_user_ref_id()
      ...
      OR (common.current_role_code()='sales' AND EXISTS(
        SELECT 1 FROM platform.team_membership own
        JOIN platform.team_membership peer ON peer.workspace_id=own.workspace_id AND peer.team_id=own.team_id
        WHERE own.workspace_id=u.workspace_id AND own.user_ref_id=common.current_user_ref_id()
          AND peer.user_ref_id=u.id AND clock_timestamp()>=own.valid_from AND clock_timestamp()<own.valid_to
          AND clock_timestamp()>=peer.valid_from AND clock_timestamp()<peer.valid_to)))
 ), events AS (
  ...
  UNION ALL SELECT *,NULL::bigint AS rank FROM group_values WHERE code='unassigned'
   ...
   'calculation','confirmed_followup_current_claim_nonlost_opportunity_multiteam_v2') INTO answer;
 RETURN answer;
END;
$$;
COMMENT ON FUNCTION security.business_ranking(text,date,date,integer[],boolean) IS
 '完整权限排名；成员按有效多部门关系纳入，事实按记录所属团队过滤且不重复；一线仅返回本人名次';
INSERT INTO ops.schema_migration(version,description) VALUES('V068','有效多部门成员排名与事实团队归属');
COMMIT;
