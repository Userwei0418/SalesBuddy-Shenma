...
DO $$ BEGIN
 IF NOT EXISTS(SELECT 1 FROM pg_roles WHERE rolname=current_user AND (rolsuper OR rolbypassrls)) THEN
  RAISE EXCEPTION 'V117 requires the audited migration owner with complete RLS visibility';
 END IF;
END $$;
...
BEGIN
  ...
  IF row_security_active('insight.business_advice'::regclass)
   ...
   OR EXISTS(SELECT 1 FROM ops.schema_migration WHERE version ~ '^V[0-9]+$'
     AND substring(version FROM 2)::int>117) THEN
  ...
  FOREACH dependent_table IN ARRAY ARRAY[
    ...
    ELSIF dependent_table='agent.conversation' AND EXISTS(SELECT 1 FROM agent.conversation
      ...
      -- Agent conversations are retained as shared history and are never
      -- deleted by this four-record archive operation.
    END IF;
  END LOOP;
...
INSERT INTO ops.schema_migration(version,description)
 VALUES('V117','固定标识飞书通知联调演示数据的一次性受控归档入口（保留共享复盘与Agent会话历史）');
COMMIT;
