...
DO $$ BEGIN
 IF NOT EXISTS(SELECT 1 FROM pg_roles WHERE rolname=current_user AND (rolsuper OR rolbypassrls)) THEN
  RAISE EXCEPTION 'V109 requires the audited migration owner with complete RLS visibility';
 END IF;
END $$;
...
CREATE FUNCTION security.feishu_notification_test_archive(p_descriptor jsonb,p_expected_plan text DEFAULT NULL) RETURNS jsonb
...
BEGIN
  ...
  IF row_security_active('insight.business_advice'::regclass)
   ...
   OR EXISTS(SELECT 1 FROM ops.schema_migration WHERE version ~ '^V[0-9]+$'
     AND substring(version FROM 2)::int>109) THEN
  ...
  SELECT c.id,c.version_no INTO cid,customer_version
   ...
   WHERE c.workspace_id=ws AND c.deleted_at IS NULL
     ...
     AND c.data_kind IN ('test','demo','unclassified')
...
REVOKE ALL ON FUNCTION security.feishu_notification_test_archive(jsonb,text) FROM PUBLIC,salegent_feishu_worker;
DO $$ DECLARE r record; BEGIN
  ...
  LOOP EXECUTE format('GRANT EXECUTE ON FUNCTION security.feishu_notification_test_archive(jsonb,text) TO %I',r.rolname); END LOOP;
END $$;
COMMENT ON FUNCTION security.feishu_notification_test_archive(jsonb,text) IS
...
INSERT INTO ops.schema_migration(version,description)
 VALUES('V109','固定标识飞书通知联调演示数据的一次性受控归档入口');
COMMIT;
