"""Current visit rules; the canonical form metadata is in contracts/visit_schema.py."""

from uuid import UUID

from sales_backend.contracts.visit_schema import (
    CONTACT_ROLES,
    CONTENT_KEYS,
    FIRST_VISIT_KEYS,
    TEXT_FIELDS,
)
from sales_backend.domain.company_rules import VisitAdmissionPolicy, visit_semantics_changed
...
def quality_grade(score, policy=None):
    return VisitAdmissionPolicy(**(policy or {})).grade(score)


def validate_content(fields):
    clean = {key: str(fields.get(key) or "").strip() for key in VISIT_FIELDS}
    for key in CONTENT_KEYS:
        if not clean[key]:
            raise ValueError(f"请补充{VISIT_FIELDS[key]}")
        if len(clean[key]) > 15000:
            raise ValueError(f"{VISIT_FIELDS[key]}过长")
    for key in set(VISIT_FIELDS) - set(CONTENT_KEYS) - {"visit_goal", "customer_main_business", "customer_needs"}:
        if len(clean[key]) > 300:
            raise ValueError(f"{VISIT_FIELDS[key]}过长")
    clean["is_first_visit"] = is_first_visit(fields)
    if clean["is_first_visit"]:
        for key in FIRST_VISIT_KEYS:
            if not clean[key]:
                raise ValueError(f"首次拜访请补充{VISIT_FIELDS[key]}")
        if clean["contact_role"] not in CONTACT_ROLES:
            raise ValueError("联系人角色请选择使用者、影响者或决策者")
    for key in ("visit_goal", "customer_main_business", "customer_needs"):
        if len(clean[key]) > 15000:
            raise ValueError(f"{VISIT_FIELDS[key]}过长")
    for key in ("opportunity_id", "source_import_id"):
        value = fields.get(key)
        clean[key] = str(UUID(str(value))) if value else None
    collaborators = fields.get("collaborator_ids") or []
    if not isinstance(collaborators, list) or len(collaborators) > 30:
        raise ValueError("协同人最多选择30人")
    clean["collaborator_ids"] = list(dict.fromkeys(str(UUID(str(x))) for x in collaborators))
    return clean
