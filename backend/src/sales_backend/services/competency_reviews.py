...
class CompetencyReviewHandler:
    ...
    def _messages(
        ...
        coaching_contract = (
            ...
            "评估仅针对本次记录呈现的能力证据，不把记录缺口归因为人员态度或实际能力缺陷。"
            "一条记录不能证明销售被动、依赖客户、忽视负责人、关系浅、情感联系不足或推进停滞；"
            "未记录不等于未行动，计划期限尚未到不能判定等待、执行不力或延误。"
            "每项不足用‘本次记录未展示某项证据’限定，不补写动机、因果或未发生事实。"
            "训练动作中的数量与时限必须明确为建议，不能声称已有会议、承诺、团队资源或客户同意。"
            "没有事实记录时，只提出可选的练习与记录方法，不要求补录不存在的过去拜访，"
            "不建立响应时限、全量补录、审批或其他新的公司管理制度。"
