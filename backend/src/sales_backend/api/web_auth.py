"""Same-origin business Web authentication. Access token stays in memory; refresh is HttpOnly."""
...
from sales_backend.api.web_identity import get_business_web_identity as get_password_identity
...
router = APIRouter(prefix="/api/v1/web/auth", tags=["Business Web authentication"])
COOKIE = "sales_business_refresh"
BINDING_COOKIE = "sales_business_session"
COOKIE_PATH = "/api/v1/web/auth"
...
async def refresh(
    ...
    try:
        ...
        session = await service.refresh(token, require_password=True, expected_session=reference, expected_channel="business_web")
