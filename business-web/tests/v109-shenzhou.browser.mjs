...
async function fixture(run,{width=1440,height=900,role='sales'}={}) {
  ...
  catch(e){await page.screenshot({path:`验收/1.0.9同步-20260925/最终定制回归/failure-${width}-${Date.now()}.png`,fullPage:true});throw e;}
...
for (const size of [{width:1440,height:900},{width:1366,height:768},{width:1024,height:600},{width:390,height:844}]) {
  test(`${size.width}px opportunity list click, eight fields, navigation and edit`,{timeout:45000},()=>fixture(async({page,go})=>{
    ...
    if(size.width===1440) await page.screenshot({path:'验收/1.0.9同步-20260925/最终定制回归/01-Web商机列表.png',fullPage:true});
    ...
    await fits(page);
    if(size.width===1440)await page.screenshot({path:'验收/1.0.9同步-20260925/最终定制回归/02-Web商机八字段.png',fullPage:true});
    if(size.width===390)await page.screenshot({path:'验收/1.0.9同步-20260925/最终定制回归/05-Web窄屏详情.png',fullPage:true});
  ...
  test(`${size.width}px follow-up select, search multi-select, cancel/remove and reload`,{timeout:60000},()=>fixture(async({page,go})=>{
    ...
    await fits(page);
    if(size.width===1440)await page.screenshot({path:'验收/1.0.9同步-20260925/最终定制回归/03-Web跟进表单.png',fullPage:true});
    if(size.width===390)await page.screenshot({path:'验收/1.0.9同步-20260925/最终定制回归/06-Web窄屏跟进.png',fullPage:true});
    await page.getByRole('button',{name:'添加业务数据',exact:true}).click();
    ...
    if(size.width===1440) {
      ...
      await page.screenshot({path:'验收/1.0.9同步-20260925/最终定制回归/04-Web业务多选.png',fullPage:true});
    }
